...
public class WordSearchII {
    ...
    public List<String> findWords(char[][] board, String[] words) {
        ...
        for (int i = 0; i < rows; i++) {
            for (int j = 0; j < rows; j++) {
                helper(board, i, j, root, stringBuilder, res, visited);
            }
        }
    ...
    private void helper(char [][] board, int x, int y, TrieNode root, StringBuilder stringBuilder,
        ...
        for (int [] dir : DIRS) {
            int neiX = dir[0] + x;
            int neiY = dir[1] + y;
            helper(board, neiX, neiY, root, stringBuilder, res, visited);
        }
        visited[x][y] = false;
